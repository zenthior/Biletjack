...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_staff'])) {
    ...
    if (empty($full_name)) {
        ...
        if (!empty($new_password)) {
            ...
            $result = $stmt->execute([$full_name, $hashed_password, $organizer_id]);
        } else {
            // Sadece ad soyad güncellenecek
            $stmt = $pdo->prepare("UPDATE qr_staff SET full_name = ? WHERE organizer_id = ?");
            $result = $stmt->execute([$full_name, $organizer_id]);
        }
    ...
    <?php else: ?>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <p>Etkinlik girişlerinde bilet doğrulama yapan yetkili hesabı</p>
            </div>
            <div class="card-body">
                ...
                <div class="staff-actions">
                    <button type="button" class="qr-btn qr-btn-success" onclick="goToQRPanel()">
...
function a(i){const C=i.parentElement.querySelector('.pw-strength');if(!C)return;function v(){const {l}=g(i.value||'');u(C,l);if(i.value){if(!(l==='m'||l==='s')){i.setCustomValidity('Lütfen en az ta orta seviye bir şifre girin.');}else{i.setCustomValidity('');}}else{i.setCustomValidity('');}}i.addEventListener('input',v);v();}
